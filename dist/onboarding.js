...
const ALL_PROVIDERS = ["anthropic", "openai", "google", "ollama"];
// ── detection ──────────────────────────────────────────────────────────────
...
function step(n, label) {
    blank();
    console.log(chalk.cyan(`  Step ${n}: `) + chalk.bold.white(label));
    hr();
}
...
export async function runOnboarding() {
    ...
    console.log(chalk.gray("  Wraps the ") +
        chalk.cyan("cascade") +
    ...
    if (chosen.name === "ollama") {
        ...
        blank();
        console.log(chalk.gray("  Get your API key here: ") +
        ...
        while (!key) {
            ...
            key = raw.trim();
            if (!key)
                console.log(chalk.red("  No key entered — please try again."));
        }
    ...
    if (models.length > 0) {
        ...
        while (true) {
            const raw = await ask(rl, chalk.green(`  Choose a model [1-${models.length}] or press Enter for recommended: `));
    ...
    blank();
    console.log(chalk.gray("  Provider : ") + chalk.white(chosen.name));
    console.log(chalk.gray("  Model    : ") + chalk.white(chosenModel));
    console.log(chalk.gray("  Config   : ") + chalk.gray("~/.config/cascade-agent/config.json"));
    blank();
    console.log(chalk.gray("  You can change your model any time:  ") +
        chalk.cyan("cascade-agent model --list"));
    console.log(chalk.gray("  Add more providers:                  ") +
        chalk.cyan("cascade-agent login"));
    blank();
    hr();
    blank();
