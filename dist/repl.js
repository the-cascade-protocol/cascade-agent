...
// ── REPL ────────────────────────────────────────────────────────────────────
export async function startRepl(initialProvider) {
    ...
    function printHeader() {
        ...
        console.log(chalk.gray("Type your request, or ") +
            chalk.cyan("/help") +
                ...
                switch (cmd) {
                    ...
                    case "/model": {
                        if (arg && arg !== "list") {
                            ...
                            provider = createProvider(config, provider.providerName, resolved);
                        ...
                        const answer = await new Promise((res) => rl.question(chalk.green(`  Choose a model [1-${models.length}] or Enter to keep current: `), res));
                        rl.resume();
                        ...
                        if (!trimmed) {
                            ...
                            const config = loadConfig();
                            provider = createProvider(config, provider.providerName, chosen);
