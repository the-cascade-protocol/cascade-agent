...
import { loadConfig, saveConfig, getApiKey, getActiveProvider, getModel, resolveModel, MODEL_ALIASES, PROVIDER_LABELS, } from "./config.js";
...
program
    ...
    if (!providerName) {
        ...
        ALL_PROVIDERS.forEach((p, i) => console.log(`  ${chalk.white(i + 1 + ".")} ${PROVIDER_LABELS[p]}`));
        console.log();
...
program
    ...
    const config = loadConfig();
    saveConfig({ ...config, activeProvider: name });
...
program
    ...
    const config = loadConfig();
    const providerName = opts?.provider ?? config.activeProvider ?? "anthropic";
    if (opts?.list) {
...
program
    ...
    requireApiKey(providerName);
    const provider = createProvider(config, providerName, opts.model ? resolveModel(opts.model) : undefined);
    if (prompt) {
        await runOneShot(provider, prompt);
    }
    else {
...
async function runOneShot(provider, prompt) {
    ...
    try {
        await runAgent(provider, [{ role: "user", content: prompt }], {
            ...
            onToolEnd: (result) => {
                process.stderr.write(chalk.gray("  ↳ " + result.split("\n").slice(0, 2).join(" ").slice(0, 120) + "\n"));
